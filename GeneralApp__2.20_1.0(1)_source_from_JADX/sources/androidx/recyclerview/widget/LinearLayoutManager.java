...
public class LinearLayoutManager extends LayoutManager implements ViewDropHandler, ScrollVectorProvider {
    ...
    /* access modifiers changed from: 0000 */
    public int scrollBy(int delta, Recycler recycler, State state) {
        if (getChildCount() == 0 || delta == 0) {
            return 0;
        }
        ensureLayoutState();
    ...
    private void recycleViewsFromStart(Recycler recycler, int scrollingOffset, int noRecycleSpace) {
    ...
    private void recycleViewsFromEnd(Recycler recycler, int scrollingOffset, int noRecycleSpace) {
        int childCount = getChildCount();
        if (scrollingOffset >= 0) {

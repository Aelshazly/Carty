...
public class GridLayoutManager extends LinearLayoutManager {
    ...
    public static abstract class SpanSizeLookup {
        ...
        public int getSpanIndex(int position, int spanCount) {
            ...
            int startPos = 0;
            if (this.mCacheSpanIndices) {
                int prevKey = findFirstKeyLessThan(this.mSpanIndexCache, position);
                if (prevKey >= 0) {
        ...
        public int getSpanGroupIndex(int adapterPosition, int spanCount) {
            ...
            int start = 0;
            if (this.mCacheSpanGroupIndices) {
                int prevKey = findFirstKeyLessThan(this.mSpanGroupIndexCache, adapterPosition);
                if (prevKey != -1) {
    ...
    public void onInitializeAccessibilityNodeInfoForItem(Recycler recycler, State state, View host, AccessibilityNodeInfoCompat info) {
        ...
        if (this.mOrientation == 0) {
            info.setCollectionItemInfo(CollectionItemInfoCompat.obtain(glp.getSpanIndex(), glp.getSpanSize(), spanGroupIndex, 1, false, false));
        } else {
            info.setCollectionItemInfo(CollectionItemInfoCompat.obtain(spanGroupIndex, 1, glp.getSpanIndex(), glp.getSpanSize(), false, false));
        }
    }
    ...
    private void assignSpans(Recycler recycler, State state, int count, boolean layingOutInPrimaryDirection) {
        int diff;
        int end;
        int start;
        if (layingOutInPrimaryDirection) {

    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new Runnable() {
                public void run() {
                    SyncQueueItem msg = C03331.this.mQueue.next();
                    while (msg != null) {
                        int i = msg.what;
                        if (i == 1) {
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new Runnable() {
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C03352.this.mQueue.next();
                        if (msg == null) {
                            C03352.this.mBackgroundRunning.set(false);
                            return;
                        }
                        int i = msg.what;
                        if (i == 1) {
                            C03352.this.mQueue.removeMessages(1);
                            callback.refresh(msg.arg1);
                        } else if (i == 2) {
                            C03352.this.mQueue.removeMessages(2);
                            C03352.this.mQueue.removeMessages(3);

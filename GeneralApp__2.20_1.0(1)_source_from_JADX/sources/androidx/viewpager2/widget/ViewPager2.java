...
import androidx.viewpager2.C1087R;
...
public final class ViewPager2 extends ViewGroup {
    ...
    private class LinearLayoutManagerImpl extends LinearLayoutManager {
        ...
        public void calculateExtraLayoutSpace(State state, int[] extraLayoutSpace) {
    ...
    class PageAwareAccessibilityProvider extends AccessibilityProvider {
        ...
        public void onAttachAdapter(Adapter<?> newAdapter) {
            updatePageAccessibilityActions();
            if (newAdapter != null) {
        ...
        public void onDetachAdapter(Adapter<?> oldAdapter) {
            if (oldAdapter != null) {
                ...
                if (itemCount != 0 && ViewPager2.this.isUserInputEnabled()) {
                    if (ViewPager2.this.getOrientation() == 0) {
                        ...
                        if (ViewPager2.this.mCurrentItem < itemCount - 1) {
                            ViewCompat.replaceAccessibilityAction(viewPager, new AccessibilityActionCompat(actionIdPageForward, null), null, this.mActionPageForward);
                        }
                        if (ViewPager2.this.mCurrentItem > 0) {
                            ViewCompat.replaceAccessibilityAction(viewPager, new AccessibilityActionCompat(actionIdPageBackward, null), null, this.mActionPageBackward);
                        }
                    } else {
                        if (ViewPager2.this.mCurrentItem < itemCount - 1) {
                            ViewCompat.replaceAccessibilityAction(viewPager, new AccessibilityActionCompat(16908359, null), null, this.mActionPageForward);
                        }
                        if (ViewPager2.this.mCurrentItem > 0) {
                            ViewCompat.replaceAccessibilityAction(viewPager, new AccessibilityActionCompat(16908358, null), null, this.mActionPageBackward);
                        }
                    }
                }
            }
        ...
        private void addCollectionInfo(AccessibilityNodeInfo info) {
            ...
            AccessibilityNodeInfoCompat.wrap(info).setCollectionInfo(CollectionInfoCompat.obtain(rowCount, colCount, false, 0));
        }
    ...
    static class SavedState extends BaseSavedState {
        public static final Creator<SavedState> CREATOR = new ClassLoaderCreator<SavedState>() {
            public SavedState createFromParcel(Parcel source, ClassLoader loader) {
                return VERSION.SDK_INT >= 24 ? new SavedState(source, loader) : new SavedState(source);
            }
    ...
    private void setOrientation(Context context, AttributeSet attrs) {
        TypedArray a = context.obtainStyledAttributes(attrs, C1087R.styleable.ViewPager2);
        if (VERSION.SDK_INT >= 29) {
            saveAttributeDataForStyleable(context, C1087R.styleable.ViewPager2, attrs, a, 0, 0);
        }
        try {
            setOrientation(a.getInt(C1087R.styleable.ViewPager2_android_orientation, 0));
        } finally {
            a.recycle();
        }
    }
    ...
    public void dispatchRestoreInstanceState(SparseArray<Parcelable> container) {
        Parcelable state = (Parcelable) container.get(getId());
        if (state instanceof SavedState) {
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        setMeasuredDimension(resolveSizeAndState(Math.max(width + getPaddingLeft() + getPaddingRight(), getSuggestedMinimumWidth()), widthMeasureSpec, childState), resolveSizeAndState(Math.max(height2, getSuggestedMinimumHeight()), heightMeasureSpec, childState << 16));
    }
    ...
    public void setCurrentItemInternal(int item, boolean smoothScroll) {
        ...
        if (adapter == null) {
            ...
            if (item2 != this.mCurrentItem || !smoothScroll) {
                ...
                if (Math.abs(((double) item2) - previousItem) > 3.0d) {

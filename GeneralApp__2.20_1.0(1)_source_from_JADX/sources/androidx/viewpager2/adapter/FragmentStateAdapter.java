...
public abstract class FragmentStateAdapter extends Adapter<FragmentViewHolder> implements StatefulAdapter {
    ...
    class FragmentMaxLifecycleEnforcer {
        ...
        public void register(RecyclerView recyclerView) {
            ...
            this.mPageChangeCallback = new OnPageChangeCallback() {
            ...
            if (!FragmentStateAdapter.this.shouldDelayFragmentTransactions() && this.mViewPager.getScrollState() == 0 && !FragmentStateAdapter.this.mFragments.isEmpty() && FragmentStateAdapter.this.getItemCount() != 0) {
                ...
                if (currentItem < FragmentStateAdapter.this.getItemCount()) {
                    ...
                    if (currentItemId != this.mPrimaryItemId || dataSetChanged) {
                        Fragment currentItemFragment = (Fragment) FragmentStateAdapter.this.mFragments.get(currentItemId);
                        if (currentItemFragment != null && currentItemFragment.isAdded()) {
                            ...
                            for (int ix = 0; ix < FragmentStateAdapter.this.mFragments.size(); ix++) {
                                ...
                                if (fragment.isAdded()) {
                                    if (itemId != this.mPrimaryItemId) {
                                        transaction.setMaxLifecycle(fragment, State.STARTED);
                                    } else {
                                        toResume = fragment;
                                    }
                            ...
                            if (toResume != null) {
                                transaction.setMaxLifecycle(toResume, State.RESUMED);
                            }
    ...
    private void ensureFragment(int position) {
        ...
        if (!this.mFragments.containsKey(itemId)) {
            ...
            newFragment.setInitialSavedState((SavedState) this.mSavedStates.get(itemId));
    ...
    public final Parcelable saveState() {
        ...
        for (int ix2 = 0; ix2 < this.mSavedStates.size(); ix2++) {
            ...
            if (containsItem(itemId2)) {
                savedState.putParcelable(createKey(KEY_PREFIX_STATE, itemId2), (Parcelable) this.mSavedStates.get(itemId2));
            }
        }
        return savedState;
    }
    ...
    public final void restoreState(Parcelable savedState) {
        ...
        for (String key : bundle.keySet()) {
            String str = KEY_PREFIX_FRAGMENT;
            if (isValidKey(key, str)) {
                this.mFragments.put(parseIdFromKey(key, str), this.mFragmentManager.getFragment(bundle, key));
            } else {
                String str2 = KEY_PREFIX_STATE;
                if (isValidKey(key, str2)) {
                    long itemId = parseIdFromKey(key, str2);
                    SavedState state = (SavedState) bundle.getParcelable(key);
                    if (containsItem(itemId)) {

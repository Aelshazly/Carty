...
public class PreferenceGroupAdapter extends Adapter<PreferenceViewHolder> implements OnPreferenceChangeInternalListener, PreferencePositionCallback {
    ...
    private ExpandButton createExpandButton(final PreferenceGroup group, List<Preference> collapsedPreferences) {
        ...
        preference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
            public boolean onPreferenceClick(Preference preference) {
                ...
                OnExpandButtonClickListener listener = group.getOnExpandButtonClickListener();
                if (listener != null) {
    ...
    public Preference getItem(int position) {
        ...
        return (Preference) this.mVisiblePreferences.get(position);
    }
    ...
    public PreferenceViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        PreferenceResourceDescriptor descriptor = (PreferenceResourceDescriptor) this.mPreferenceResourceDescriptors.get(viewType);
        LayoutInflater inflater = LayoutInflater.from(parent.getContext());
        TypedArray a = parent.getContext().obtainStyledAttributes(null, C1048R.styleable.BackgroundStyle);
        Drawable background = a.getDrawable(C1048R.styleable.BackgroundStyle_android_selectableItemBackground);
        if (background == null) {
    ...
    public int getPreferenceAdapterPosition(String key) {
        ...
        for (int i = 0; i < size; i++) {
            if (TextUtils.equals(key, ((Preference) this.mVisiblePreferences.get(i)).getKey())) {
                return i;
            }
        }
        return -1;
    ...
    public int getPreferenceAdapterPosition(Preference preference) {
        int size = this.mVisiblePreferences.size();
        for (int i = 0; i < size; i++) {
            Preference candidate = (Preference) this.mVisiblePreferences.get(i);

...
public final class ErrorCorrection {
    ...
    public int decode(int[] received, int numECCodewords, int[] erasures) throws ChecksumException {
        ...
        if (iArr2 != null) {
            ...
            int i4 = 0;
            while (i4 < length) {
                int b = this.field.exp((iArr.length - i3) - iArr2[i4]);
                ModulusGF modulusGF = this.field;
                knownErrors2 = knownErrors2.multiply(new ModulusPoly(modulusGF, new int[]{modulusGF.subtract(0, b), 1}));
                i4++;
                i3 = 1;
            }
            ModulusPoly modulusPoly = knownErrors2;
        }
        ModulusPoly[] runEuclideanAlgorithm = runEuclideanAlgorithm(this.field.buildMonomial(i, 1), new ModulusPoly(this.field, S), i);
        ModulusPoly[] sigmaOmega = runEuclideanAlgorithm;
        ModulusPoly sigma = runEuclideanAlgorithm[0];
        ModulusPoly omega = sigmaOmega[1];
        int[] errorLocations = findErrorLocations(sigma);
        int[] errorMagnitudes = findErrorMagnitudes(omega, sigma, errorLocations);
        int i5 = 0;
        while (i5 < errorLocations.length) {
            ModulusPoly poly2 = poly;
            int length2 = (iArr.length - 1) - this.field.log(errorLocations[i5]);
            int i6 = position;
            position = length2;
            if (length2 >= 0) {
                iArr[position] = this.field.subtract(iArr[position], errorMagnitudes[i5]);
                i5++;
                int i7 = numECCodewords;
                int[] iArr3 = erasures;
                poly = poly2;
            } else {

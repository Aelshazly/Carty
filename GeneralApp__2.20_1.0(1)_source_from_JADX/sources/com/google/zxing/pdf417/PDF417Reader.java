...
public final class PDF417Reader implements Reader, MultipleBarcodeReader {
    ...
    public Result decode(BinaryBitmap image, Map<DecodeHintType, ?> hints) throws NotFoundException, FormatException, ChecksumException {
        Result[] decode = decode(image, hints, false);
        Result[] result = decode;
        if (decode != null && result.length != 0 && result[0] != null) {
            return result[0];
        }
    ...
    private static Result[] decode(BinaryBitmap image, Map<DecodeHintType, ?> hints, boolean multiple) throws NotFoundException, FormatException, ChecksumException {
        ...
        PDF417DetectorResult detect = Detector.detect(image, hints, multiple);
        PDF417DetectorResult detectorResult = detect;
        for (ResultPoint[] points : detect.getPoints()) {
            ...
            PDF417ResultMetadata pdf417ResultMetadata = pDF417ResultMetadata;

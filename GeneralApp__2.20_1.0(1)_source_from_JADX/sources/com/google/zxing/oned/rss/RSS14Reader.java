...
public final class RSS14Reader extends AbstractRSSReader {
        ...
        for (Pair pair : this.possibleLeftPairs) {
            ...
            if (pair.getCount() > 1) {
                for (Pair pair2 : this.possibleRightPairs) {
                    Pair right = pair2;
                    if (pair2.getCount() > 1 && checkChecksum(left, right)) {
    ...
    private DataCharacter decodeDataCharacter(BitArray row, FinderPattern pattern, boolean outsideChar) throws NotFoundException {
        ...
        if (!z) {
            ...
            int i9 = oddWidest3;
            int oddWidest4 = RSSUtils.getRSSvalue(evenCounts, evenWidest2, true);
            int i10 = evenWidest2;
            int i11 = oddWidest4;
            return new DataCharacter((vOdd2 * OUTSIDE_EVEN_TOTAL_SUBSET[group2]) + oddWidest4 + OUTSIDE_GSUM[group2], checksumPortion);
        }
    }

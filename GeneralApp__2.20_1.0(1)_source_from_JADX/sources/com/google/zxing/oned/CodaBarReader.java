...
public final class CodaBarReader extends OneDReader {
    ...
    public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> hints) throws NotFoundException {
        ...
        for (int i3 = -8; i3 < -1; i3++) {
            lastPatternSize += codaBarReader.counters[nextStart + i3];
        }
        if (nextStart >= codaBarReader.counterLength || trailingWhitespace >= lastPatternSize / 2) {
            codaBarReader.validatePattern(startOffset);
            for (int i4 = 0; i4 < codaBarReader.decodeRowResult.length(); i4++) {
                StringBuilder sb = codaBarReader.decodeRowResult;
                sb.setCharAt(i4, ALPHABET[sb.charAt(i4)]);
            }
            if (arrayContains(STARTEND_ENCODING, codaBarReader.decodeRowResult.charAt(i2))) {
                ...
                if (!arrayContains(STARTEND_ENCODING, sb2.charAt(sb2.length() - 1))) {
                    ...
                    if (map == null || !map.containsKey(DecodeHintType.RETURN_CODABAR_START_END)) {
                        ...
                        codaBarReader.decodeRowResult.deleteCharAt(i2);
                    }
                    int runningCount = 0;
                    for (int i5 = 0; i5 < startOffset; i5++) {
                        runningCount += codaBarReader.counters[i5];
                    }
                    float left = (float) runningCount;
                    for (int i6 = startOffset; i6 < nextStart - 1; i6++) {
                        runningCount += codaBarReader.counters[i6];
                    }
                    float right = (float) runningCount;
                    return new Result(codaBarReader.decodeRowResult.toString(), null, new ResultPoint[]{new ResultPoint(left, (float) i), new ResultPoint(right, (float) i)}, BarcodeFormat.CODABAR);
                } else {
                    throw NotFoundException.getNotFoundInstance();
                }
            } else {

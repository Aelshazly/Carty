...
public class BottomSheetBehavior<V extends View> extends Behavior<V> {
    ...
    private static final int DEF_STYLE_RES = C1124R.style.Widget_Design_BottomSheet_Modal;
    ...
    public BottomSheetBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.obtainStyledAttributes(attrs, C1124R.styleable.BottomSheetBehavior_Layout);
        this.shapeThemingEnabled = a.hasValue(C1124R.styleable.BottomSheetBehavior_Layout_shapeAppearance);
        boolean hasBackgroundTint = a.hasValue(C1124R.styleable.BottomSheetBehavior_Layout_backgroundTint);
        if (hasBackgroundTint) {
            createMaterialShapeDrawable(context, attrs, hasBackgroundTint, MaterialResources.getColorStateList(context, a, C1124R.styleable.BottomSheetBehavior_Layout_backgroundTint));
        } else {
            createMaterialShapeDrawable(context, attrs, hasBackgroundTint);
        }
        createShapeValueAnimator();
        if (VERSION.SDK_INT >= 21) {
            this.elevation = a.getDimension(C1124R.styleable.BottomSheetBehavior_Layout_android_elevation, -1.0f);
        }
        TypedValue value = a.peekValue(C1124R.styleable.BottomSheetBehavior_Layout_behavior_peekHeight);
        if (value == null || value.data != -1) {
            setPeekHeight(a.getDimensionPixelSize(C1124R.styleable.BottomSheetBehavior_Layout_behavior_peekHeight, -1));
        } else {
            setPeekHeight(value.data);
        }
        setHideable(a.getBoolean(C1124R.styleable.BottomSheetBehavior_Layout_behavior_hideable, false));
        setFitToContents(a.getBoolean(C1124R.styleable.BottomSheetBehavior_Layout_behavior_fitToContents, true));
        setSkipCollapsed(a.getBoolean(C1124R.styleable.BottomSheetBehavior_Layout_behavior_skipCollapsed, false));
        setSaveFlags(a.getInt(C1124R.styleable.BottomSheetBehavior_Layout_behavior_saveFlags, 0));
        setHalfExpandedRatio(a.getFloat(C1124R.styleable.BottomSheetBehavior_Layout_behavior_halfExpandedRatio, HIDE_THRESHOLD));
        setExpandedOffset(a.getInt(C1124R.styleable.BottomSheetBehavior_Layout_behavior_expandedOffset, 0));
        a.recycle();
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, V child, int layoutDirection) {
        ...
        if (this.viewRef == null) {
            this.peekHeightMin = parent.getResources().getDimensionPixelSize(C1124R.dimen.design_bottom_sheet_peek_height_min);
    ...
    public boolean onNestedPreFling(CoordinatorLayout coordinatorLayout, V child, View target, float velocityX, float velocityY) {
        ...
        if (target == weakReference.get() && (this.state != 3 || super.onNestedPreFling(coordinatorLayout, child, target, velocityX, velocityY))) {
            z = true;
        }
        return z;
    }
    ...
    public void setStateInternal(int state2) {
        if (this.state != state2) {
            ...
            if (weakReference != null) {
                View bottomSheet = (View) weakReference.get();
                if (bottomSheet != null) {
                    ...
                    for (int i = 0; i < this.callbacks.size(); i++) {
                        ((BottomSheetCallback) this.callbacks.get(i)).onStateChanged(bottomSheet, state2);
                    }
                    updateAccessibilityActions();
                }
            }
    ...
    private void createMaterialShapeDrawable(Context context, AttributeSet attrs, boolean hasBackgroundTint, ColorStateList bottomSheetColor) {
        if (this.shapeThemingEnabled) {
            this.shapeAppearanceModelDefault = ShapeAppearanceModel.builder(context, attrs, C1124R.attr.bottomSheetStyle, DEF_STYLE_RES).build();
    ...
    public void dispatchOnSlide(int top) {
        ...
        if (bottomSheet != null && !this.callbacks.isEmpty()) {
            ...
            for (int i2 = 0; i2 < this.callbacks.size(); i2++) {
                ((BottomSheetCallback) this.callbacks.get(i2)).onSlide(bottomSheet, slideOffset);
            }
        }
    }
        ...
        if (weakReference != null) {
            ...
            if (child != null) {
                ...
                if (this.hideable && this.state != 5) {
                    addAccessibilityActionForState(child, AccessibilityActionCompat.ACTION_DISMISS, 5);
                }
                int i = this.state;
                int i2 = 6;
                if (i == 3) {
                    ...
                    addAccessibilityActionForState(child, AccessibilityActionCompat.ACTION_COLLAPSE, i2);
                } else if (i == 4) {
                    if (this.fitToContents) {
                        i2 = 3;
                    }
                    addAccessibilityActionForState(child, AccessibilityActionCompat.ACTION_EXPAND, i2);
                } else if (i == 6) {
                    addAccessibilityActionForState(child, AccessibilityActionCompat.ACTION_COLLAPSE, 4);
                    addAccessibilityActionForState(child, AccessibilityActionCompat.ACTION_EXPAND, 3);
                }
            }
        }
    }

    private void addAccessibilityActionForState(V child, AccessibilityActionCompat action, final int state2) {

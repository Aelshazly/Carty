...
public class Chip extends AppCompatCheckBox implements Delegate, Shapeable {
    ...
    private class ChipTouchHelper extends ExploreByTouchHelper {
        ...
        public void getVisibleVirtualViews(List<Integer> virtualViewIds) {
            ...
            if (Chip.this.hasCloseIcon() && Chip.this.isCloseIconVisible()) {
                virtualViewIds.add(Integer.valueOf(1));
            }
        }
        ...
        public void onPopulateNodeForVirtualView(int virtualViewId, AccessibilityNodeInfoCompat node) {
            ...
            if (virtualViewId == 1) {
                ...
                if (closeIconContentDescription != null) {
                    ...
                    int i = C1124R.string.mtrl_chip_close_icon_content_description;
    ...
    public Chip(Context context, AttributeSet attrs) {
        this(context, attrs, C1124R.attr.chipStyle);
    }
    ...
    public Chip(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        ChipDrawable drawable = ChipDrawable.createFromAttributes(context, attrs, defStyleAttr, C1124R.style.Widget_MaterialComponents_Chip_Action);
        initMinTouchTarget(context, attrs, defStyleAttr);
        setChipDrawable(drawable);
        drawable.setElevation(ViewCompat.getElevation(this));
        TypedArray a = ThemeEnforcement.obtainStyledAttributes(context, attrs, C1124R.styleable.Chip, defStyleAttr, C1124R.style.Widget_MaterialComponents_Chip_Action, new int[0]);
        if (VERSION.SDK_INT < 23) {
            setTextColor(MaterialResources.getColorStateList(context, a, C1124R.styleable.Chip_android_textColor));
        }
        boolean hasShapeAppearanceAttribute = a.hasValue(C1124R.styleable.Chip_shapeAppearance);
        a.recycle();
    ...
    private void initMinTouchTarget(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray a = ThemeEnforcement.obtainStyledAttributes(context, attrs, C1124R.styleable.Chip, defStyleAttr, C1124R.style.Widget_MaterialComponents_Chip_Action, new int[0]);
        this.ensureMinTouchTargetSize = a.getBoolean(C1124R.styleable.Chip_ensureMinTouchTargetSize, false);
        this.minTouchTargetSize = (int) Math.ceil((double) a.getDimension(C1124R.styleable.Chip_chipMinTouchTargetSize, (float) Math.ceil((double) ViewUtils.dpToPx(getContext(), 48))));
        a.recycle();
    }
    ...
    private void insetChipBackgroundDrawable(int insetLeft, int insetTop, int insetRight, int insetBottom) {
        InsetDrawable insetDrawable = new InsetDrawable(this.chipDrawable, insetLeft, insetTop, insetRight, insetBottom);

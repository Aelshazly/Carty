...
final class SchemaManager extends SQLiteOpenHelper {
    ...
    private static final List<Migration> INCREMENTAL_MIGRATIONS = Arrays.asList(new Migration[]{MIGRATE_TO_V1, MIGRATE_TO_V2, MIGRATE_TO_V3});
    ...
    SchemaManager(Context context, @Named("SCHEMA_VERSION") int schemaVersion2) {
        super(context, DB_NAME, null, schemaVersion2);
    ...
    private void upgrade(SQLiteDatabase db, int fromVersion, int toVersion) {
        if (toVersion <= INCREMENTAL_MIGRATIONS.size()) {
            for (int version = fromVersion; version < toVersion; version++) {
                ((Migration) INCREMENTAL_MIGRATIONS.get(version)).upgrade(db);
            }
            return;
        }
